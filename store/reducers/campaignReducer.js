	...
	switch (action.type) {
		...
		case actionTypes.DELETE_STORY_AD_CARD:
			...
			deleteStoryAds = deleteStoryAds.filter(ad => {
				if (
					(action.payload.card.hasOwnProperty('item') && action.payload.card.item.id !== ad.id) ||
					(action.payload.hasOwnProperty('data') && ad.story_id !== action.payload.data.story_id)
				)
					return ad;
			});
		...
		case actionTypes.SET_REJECTED_STORYADS:
			...
			oldStoryAdsArray = [
				...oldStoryAdsArray,
				{
					id: parseInt(oldStoryAdsArray[oldStoryAdsArray.length - 1].story_id) + 1,
					call_to_action: { label: 'BLANK', value: 'BLANK' },
					media: '//',
					destination: 'BLANK',
					attachment: 'BLANK',
		...
		case actionTypes.SET_REJECTED_COLLECTIONADS:
			...
			return {
				...
				collectionAdLinkForm: action.payload[0].interaction_type === 'WEB_VIEW' ? 1 : 2,
			};
