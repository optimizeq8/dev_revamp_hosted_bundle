...
import { Content, Text, Item, Input, Container, Icon, Label } from 'native-base';
...
class AdObjective extends Component {
	...
	_handleSubmission = () => {
		...
		if (!nameError && !objectiveError && !dateErrors.start_timeError && !dateErrors.end_timeError) {
			...
			else this.props.ad_objective({ campaign_id: 0, ...this.state.campaignInfo }, this.props.navigation);
		}
	};
			...
			<SafeAreaView style={styles.safeAreaView} forceInset={{ bottom: 'never', top: 'always' }}>
				...
				<TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
					<Container style={styles.container}>
						<BackdropIcon style={styles.backDrop} height={heightPercentageToDP('100%')} />
						<CustomHeader
							closeButton={false}
							segment={{
