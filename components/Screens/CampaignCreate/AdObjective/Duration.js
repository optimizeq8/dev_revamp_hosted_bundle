...
export default class Duration extends Component {
	render() {
		let currentDay = dateFormat(new Date(), 'd mmm').toUpperCase() + ' ' + new Date().getFullYear();
		let end_time = '';
		let start_time = '';
		let end_year = '';
		let start_year = '';
		...
		return (
			...
			>
				<View style={styles.dateContainer}>
					...
					{!this.props.slidePanel || new Date(selectedCampaign.end_time) < new Date() ? (
						<View style={styles.dateColumn}>
						...
						this.props.slidePanel && (
							<View style={styles.dateColumn}>
								<Text style={styles.dateLabel}>End</Text>
								<Text style={styles.date}>
									{new Date(selectedCampaign.start_time.split('T')[0]) >= new Date()
										? end_time + ' ' + end_year
										: currentDay}
								</Text>
							</View>
