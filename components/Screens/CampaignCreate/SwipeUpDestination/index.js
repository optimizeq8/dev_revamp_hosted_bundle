...
class SwipeUpDestination extends Component {
	...
	componentDidMount() {
		...
		this.setState({
			media,
			selected: this.props.data ? this.props.data.destination : destination ? destination : '',
		});
			...
			() => {
				Segment.trackWithProperties('Selected ' + value + ' Swipeup', {
					...
					label: this.props.data
						...
						: this.props.navigation.getParam('objective', 'objective') + ' Objective',
				});
			}
		);
	};
			...
			case 'REMOTE_WEBPAGE': {
				menu = (
					<Website
						...
						_changeDestination={this.props.navigation.state.params._changeDestination}
