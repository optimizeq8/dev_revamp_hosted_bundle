...
export default class EditProductDetail extends React.Component {
	...
	componentDidMount() {
		...
		this.setState({
			item: item,
			cartList: list,
			productNameError: !item.productName || (item.productName && item.productName.length === 0),
			priceError: !item.price || (item.price && item.price.length === 0),
		});
	}
	_handleSubmission = () => {
		const newList = [...this.state.cartList];
		const index = findIndex(newList, item => item.imageId === this.state.item.imageId);
		newList[index] = this.state.item;
		// console.log('newList', newList);

		this.props.navigation.navigate('SelectedInstagramProductsList', { selectetedItems: newList });
	};
	render() {
		return (
