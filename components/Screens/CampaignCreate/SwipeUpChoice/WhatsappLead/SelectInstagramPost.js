...
class SelectInstagramPost extends React.Component {
	...
	componentDidMount() {
		...
		this.props.getInstagramPost(this.props.data.insta_handle);
		this.props.getWebProducts(this.props.data.campaign_id);
	}

	componentDidUpdate(prevProps, prevState) {
		if (prevProps.getWebProductsLoading && !this.props.getWebProductsLoading && !this.props.savingWebProducts) {
			this.setState({
	...
	addToList = item => {
		...
		const checkifALreadyExist = find(this.state.cartList, it => it.imageId === item.imageId);
		...
		return (
			<SafeAreaView forceInset={{ top: 'always', bottom: 'never' }} style={styles.safeAreaContainer}>
				...
				<Container style={styles.container}>
					<CustomeHeader title={'Whatsapp Campaign'} closeButton={false} navigation={this.props.navigation} />
					<Content
						style={{
							paddingTop: 20,
					...
					>
						...
						{!this.props.instagramPostLoading && !isEmpty(this.props.instagramPostList) && (
							<Text
								style={{
									fontFamily: 'montserrat-regular',
									color: '#FFF',
						...
						{!this.props.instagramPostLoading && !isEmpty(this.props.instagramPostList) && (
							<View style={styles.bottonViewWebsite}>
								<LowerButton checkmark={true} bottom={0} function={this._handleSubmission} />
							</View>
						)}
					</Content>
				</Container>
...
const mapDispatchToProps = dispatch => ({
	...
	getWebProducts: campaign_id => dispatch(actionCreators.getWebProducts(campaign_id)),
	getInstagramPost: insta_handle => dispatch(actionCreators.getInstagramPost(insta_handle)),
});
