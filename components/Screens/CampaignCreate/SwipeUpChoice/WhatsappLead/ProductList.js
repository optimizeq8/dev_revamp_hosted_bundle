...
class ProductList extends React.Component {
	...
	editItem = item => {
		this.props.navigation.navigate('EditProductDetailInstagramPost', { cartList: this.state.cartList, item: item });
	};
	_handleSubmission = () => {
		...
		return (
			<SafeAreaView forceInset={{ top: 'always', bottom: 'never' }} style={styles.safeAreaContainer}>
				<NavigationEvents
					onDidFocus={() => {
						const list = this.props.navigation.getParam('selectetedItems', []);
						const checkIfHasKeyProductName = list.map(item => item.hasOwnProperty('productName'));
								...
								return (
									...
									>
										...
										>
											...
											>
												<PenIcon style={{ width: 30, height: 30 }} width={30} height={30} />
											</View>
										</View>
										<Text
											style={{
