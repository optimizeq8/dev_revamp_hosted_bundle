...
class App extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      transitionToApp: new Animated.Value(0),
      animDone: false,
  ...
  render() {
    ...
    let opacity = {
      opacity: this.state.transitionToApp.interpolate({
        inputRange: [0, 25, 50],
        outputRange: [0, 0, 1],
        extrapolate: "clamp",
      }),
      transform: [
        {
          scale: this.state.transitionToApp.interpolate({
            inputRange: [0, 100],
            outputRange: [1.1, 1],
          }),
        },
    ...
    if (!this.state.isLoadingComplete) {
      console.log("i;m here", !this.state.animDone);
      return (
        <View>
          <StatusBar
  ...
  _handleFinishLoading = () => {
    this.setState({ isLoadingComplete: true });
    Animated.timing(this.state.transitionToApp, {
      toValue: 100,
      duration: 1000,
